...
export default function App() {
  ...
  const totalMoviesWatched = movies.filter((movie) => movie.watched===true).length;
  const totalMoviesUnwatched = movies.filter((movie) => movie.watched=== false).length;
  
  return (
    <div>
      <h1>Movie Night</h1>
      <p>Total: {totalMovies} | Watched: {totalMoviesUnwatched} | Unwatched: {totalMoviesWatched}
      </p>
      {movies.map((movie) => (
